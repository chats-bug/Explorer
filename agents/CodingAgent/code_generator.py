...
class CodeGenerator:
    def __init__(
        ...
        self.system_prompt = open("prompts/code_generator_prompt.txt", "r").read()
    ...
    def run(self, plan: Dict, file_paths: List[str]):
        files_and_code_string = self._prepare_files(file_paths=file_paths)
        messages = [
            {"role": "system", "content": self.system_prompt},
            {
                "role": "user",
                "content": f"The plan for the problem statement is: {json.dumps(plan, indent=4)}",
            },
            {
                "role": "user",
